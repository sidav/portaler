...
func (rb *RaylibBackend) DrawRect(x, y, w, h int) {
	rl.DrawRectangleLines(int32(x), int32(y), int32(w), int32(h), rb.currColor)
}
...
func (rb *RaylibBackend) DrawLine(x, y, tx, ty int) {
	rl.DrawLine(int32(x), int32(y), int32(tx), int32(ty), rb.currColor)
}
